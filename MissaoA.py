...
async def run(chassi: Chassi, anexo: Anexo, hub: Hub): 
    await chassi.seguirReto(750, Speed.VERY_FAST) 
    await chassi.virar(43, Speed.VERY_FAST)

    await multitask(
        chassi.seguirReto(140, Speed.SLOW),
        anexo.girarAmbos(-330, 300, -700)
    )

    await anexo.girarAmbos(-350, 350,400) # Levantar
    await wait(200)
    await chassi.seguirReto(-45, Speed.VERY_FAST)


    await multitask(
        chassi.virar(-75, Speed.SLOW), #fazer pedras
        wait(2000),
        race=True
    )
    ...
    await chassi.virar(-60, Speed.VERY_FAST)
    await chassi.seguirReto(-350, Speed.VERY_FAST)  # Da ré para empurrar as pedras
    
    await chassi.seguirReto(220,Speed.VERY_FAST)
    await chassi.virar(-42, Speed.VERY_FAST) #virar e ajeitar
